...
const AdminIssuesIndex = () => {
  ...
  const loadIssues = async () => {
    ...
    setIssues(data.sort((a, b) => b.createdAt.localeCompare(a.createdAt)));
  };
  console.log(issues);
  ...
  const solvedIssue = async (issueId) => {
    setLoading(true);
    try {
      const { data } = await axios.put(`/api/admin/issue/solved/${issueId}`);
      loadIssues();
      // console.log("ISSUE RESOLVED =>", data);
      toast("Issue Solved");
      setLoading(false);
